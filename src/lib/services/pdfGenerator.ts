...
class SecondPageGenerator extends PDFPageGenerator {
  protected async generateContent(): Promise<void> {
    ...
    yPosition -= 40;
    const noticeText = 'Hat der*die Teilnehmende zu Beginn der Konferenz am 6. März 2025 das 18. Lebensjahr noch nicht vollendet, so müssen ZUSÄTZLICH zur Unterschrift der*des Teilnehmenden auf Seite 1 auch die/der Erziehungsberechtigte(n) diese Einverständniserklärung unterzeichnen.';
    ...
    // Add consent text
    yPosition -= 35;
    const consentText = 'im folgenden "Teilnehmende*r", das 18. Lebensjahr am 6. März 2025 noch nicht vollendet hat, erkläre(n) wir/ich uns/mich in meiner Funktion als Erziehungsberechtigte*r mit der Teilnahme des*der Teilnehmenden an MUN-SH 2025 einverstanden und akzeptiere(n) die beigefügten Vertragsbedingungen und die Datenschutzerklärung und in Vertretung von des*der Teilnehmenden.';
    yPosition = this.utils.drawWrappedText(consentText, yPosition);

    // Add supervision notice
    yPosition -= 35;
    const supervisionText = 'Mir ist bewusst, dass DMUN e.V. als Veranstalter keine Aufsichtspflicht obliegt. Vor allem außerhalb der Tagungszeit und außerhalb des Tagungsortes bzw. außerhalb der von DMUN e.V. angebotenen Abendveranstaltungen handeln die Teilnehmenden eigenverantwortlich; eine Haftung durch DMUN e.V. für das Verhalten von Teilnehmenden ist nicht möglich. Auf allen von DMUN e.V. angebotenen Veranstaltungen haben die Teilnehmenden den Anordnungen des Organisationsteams Folge zu leisten. Für die An- und Abfahrt zu den Veranstaltungsgebäuden sowie für das Geschehen in den Unterkünften sind die Teilnehmenden selbst verantwortlich.';
    yPosition = this.utils.drawWrappedText(supervisionText, yPosition);

    // Add late hour notice
    yPosition -= 35;
    const lateHoursText = 'Uns/Mir ist des weiteren bekannt, dass sich der*die Teilnehmende während der Abendveranstaltung möglicherweise länger als 22 bzw. 24 Uhr außerhalb der Unterkunft bewegt und auf den Wegen von und zu den Tagungsorten nicht beaufsichtigt wird.';
...
class ThirdPageGenerator extends PDFPageGenerator {
  protected async generateContent(): Promise<void> {
    ...
    yPosition -= 30;
    const mainText = 'EINWILLIGUNG zur Verwendung meines Vor- und Nachnamens und Bildnisses in Video oder als Foto durch die Model United Nations e. V. (DMUN) als Trägerverein von Model United Nations Baden-Württemberg (MUNBW).';
    ...
    yPosition -= 30;
    const consentIntro = 'ein, dass DMUN meinen vorstehenden Vor- und Nachnamen sowie mein Bildnis in folgender Form verwendet (zutreffendes bitte ankreuzen!):';
    ...
    yPosition -= 25;
    const purposesIntro = 'Die Nutzung meines Vor- und Nachnamens und Bildnisses im vorgenannten Foto/Video erfolgt zu folgenden Zwecken (zutreffendes bitte ankreuzen!):';
...
class FourthPageGenerator extends PDFPageGenerator {
  protected async generateContent(): Promise<void> {
    ...
    yPosition -= 30;
    const rightsText = 'Die vorstehenden Befugnisse sind unentgeltlich. Sie gelten für alle bekannten und unbekannten Nutzungsarten. Ein Verkauf meines Bildnisses ist nicht erlaubt.';
...
class FifthPageGenerator extends PDFPageGenerator {
  protected async generateContent(): Promise<void> {
    ...
    yPosition -= 60;
    this.page.drawText('Bei Minderjährigen (zusätzlich!): Bestätigung durch die gesetzlichen Vertreter:', {
      x: this.styles.margin.left,
      y: yPosition,
      size: this.styles.fontSize.heading,
      font: this.helveticaBold,
    ...
    yPosition -= 20;
    const guardianDesignation = 'des Vaters bzw. der zweiten sorge- und erziehungsberechtigten Person] bestätigen hiermit,';
    yPosition = this.utils.drawWrappedText(guardianDesignation, yPosition);

    // Add confirmation text
    yPosition -= 30;
    const confirmationText = 'dass ich/wir der/die gesetzliche(n) Vertreter/-in der vorstehend genannten teilnehmenden Person bin/sind.';
    yPosition = this.utils.drawWrappedText(confirmationText, yPosition);

    // Add consent text
    yPosition -= 30;
    const consentText = 'Als gesetzliche Vertreter stimmen wir der vorstehenden Einwilligungserklärung unseres Kindes zu.';
    yPosition = this.utils.drawWrappedText(consentText, yPosition);
